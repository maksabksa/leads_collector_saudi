...
${realData ? "ملاحظة: لديك بيانات حقيقية من الحساب — استخدمها لتقديم تحليل دقيق وعميق." : "ملاحظة: قيّم الحساب بناءً على المعرفة العامة بحسابات هذا النوع من الأنشطة في السعودية."}`;
      ...
      // أخذ Screenshot لإنستغرام بالتوازي مع حفظ البيانات
      let igScreenshotUrl: string | undefined;
      try {
        const igBuf = await takeSocialMediaScreenshot(input.profileUrl, "instagram", 30000);
        if (igBuf) {
          const suffix = Math.random().toString(36).slice(2, 8);
          const key = `screenshots/social-${input.leadId}-instagram-${suffix}.png`;
          const { url: s3Url } = await storagePut(key, igBuf, "image/png");
          igScreenshotUrl = s3Url;
          console.log(`[Screenshot] Instagram saved: ${s3Url}`);
        }
      } catch (ssErr: any) {
        console.warn("[Screenshot] Instagram failed (non-blocking):", ssErr?.message);
      }
...
${realData ? "ملاحظة: لديك بيانات حقيقية — استخدمها لتقديم تحليل دقيق وعميق." : "قيّم بناءً على المعرفة العامة بهذا النوع من الأنشطة في السعودية."}`;
      ...
      // أخذ Screenshot لتويتر
      let twScreenshotUrl: string | undefined;
      try {
        const twBuf = await takeSocialMediaScreenshot(input.profileUrl, "twitter", 30000);
        if (twBuf) {
          const suffix = Math.random().toString(36).slice(2, 8);
          const key = `screenshots/social-${input.leadId}-twitter-${suffix}.png`;
          const { url: s3Url } = await storagePut(key, twBuf, "image/png");
          twScreenshotUrl = s3Url;
          console.log(`[Screenshot] Twitter saved: ${s3Url}`);
        }
      } catch (ssErr: any) {
        console.warn("[Screenshot] Twitter failed (non-blocking):", ssErr?.message);
      }
...
${realData ? "ملاحظة: لديك بيانات حقيقية — استخدمها لتقديم تحليل دقيق وعميق." : "قيّم بناءً على المعرفة العامة بحسابات هذا النوع في السعودية."}`;
      ...
      // التقاط سكرينشوت تيك توك لإدراجه في التقرير
      let tiktokScreenshotUrl: string | undefined;
      try {
        const tiktokBuf = await takeSocialMediaScreenshot(input.profileUrl, "tiktok", 30000);
        if (tiktokBuf) {
          const suffix = Math.random().toString(36).slice(2, 8);
          const key = `screenshots/social-${input.leadId}-tiktok-${suffix}.png`;
          const { url: s3Url } = await storagePut(key, tiktokBuf, "image/png");
          tiktokScreenshotUrl = s3Url;
          console.log(`[Screenshot] TikTok saved: ${s3Url}`);
        }
      } catch (ssErr: any) {
        console.warn("[Screenshot] TikTok failed (non-blocking):", ssErr?.message);
      }
      ...
      const analysisId = await createSocialAnalysis({
        ...
        recommendations: analysis.recommendations ?? [],
        rawAnalysis: content,
        screenshotUrl: tiktokScreenshotUrl,
      });
